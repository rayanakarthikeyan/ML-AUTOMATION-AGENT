...
DEFAULT_GITHUB_REPO = "huggingface/ml-automation-agent"
DEFAULT_HF_SPACE = "smolagents/ml-automation-agent"
...
PM_SYSTEM_PROMPT = """You are a senior product manager for ML Automation Agent.
...
def _github_headers(token: str | None) -> dict[str, str]:
    headers = {
        ...
        "User-Agent": "ml-automation-agent-backlog-prioritizer",
    }
    if token:
...
def render_markdown_report(
    ...
    *,
    ...
    lines = ["# ML Automation Agent Backlog Prioritization", ""]
    if generated_at:
...
def default_github_issue_title(generated_at: str) -> str:
    ...
    return f"ML Automation Agent backlog prioritization report - {date_text}"
