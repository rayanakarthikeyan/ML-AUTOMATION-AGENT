...
the README so the embedded iframe in ml-automation-agent renders without a login click —
...
Embedded trackio dashboard for ml-automation-agent runs.
"""

_REQUIREMENTS = "trackio\n"
_APP_PY = "import trackio\ntrackio.show()\n"

# ml-automation-agent brand mark surfaced inside the trackio dashboard. Trackio reads
...
# copy on the smolagents/ml-automation-agent Space repo so any seeded dashboard inherits
# the ml-automation-agent branding without each user having to host the asset.
_LOGO_URL = (
    "https://huggingface.co/spaces/smolagents/ml-automation-agent/"
    ...
    if _already_seeded(api, space_id):
        ...
        for path_in_repo, content in _FILES.items():
            api.upload_file(
                ...
                repo_id=space_id,
                repo_type="space",
                commit_message=f"ml-automation-agent: seed trackio dashboard ({path_in_repo})",
            )
        seeded_files = True
