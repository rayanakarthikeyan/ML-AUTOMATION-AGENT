...
* ``row`` — single-line JSONL row used by the existing org telemetry/KPI
    pipeline (``smolagents/ml-automation-agent-sessions``). Compatible with
...
* ``claude_code`` — one event per line in the Claude Code JSONL schema,
  ...
  (https://huggingface.co/changelog/agent-trace-viewer). Used for the
    per-user private dataset (default ``{hf_user}/ml-automation-agent-sessions``).
"""
...
_PERSONAL_TOKEN_ENV = "_ML_AUTOMATION_AGENT_PERSONAL_TOKEN"
...
def _resolve_token(token_env: str | None) -> str:
    ...
    if token_env == "HF_TOKEN":
        try:
            ...
            personal_env = os.environ.get(_PERSONAL_TOKEN_ENV) or os.environ.get("_ML_INTERN_PERSONAL_TOKEN")
            return (
                resolve_hf_token(
                    personal_env,
            ...
            token = os.environ.get(_PERSONAL_TOKEN_ENV) or os.environ.get("_ML_INTERN_PERSONAL_TOKEN") or os.environ.get("HF_TOKEN")
            return token or ""
...
def dataset_card_readme(repo_id: str) -> str:
    """Dataset card for personal ML Automation Agent session trace repos."""
    return """---
pretty_name: "ML Automation Agent Session Traces"
language:
- en
license: other
task_categories:
...
# ML Automation Agent session traces

This dataset contains ML Automation Agent coding agent session traces uploaded from local
ML Automation Agent runs. The traces are stored as JSON Lines files under `sessions/`,
with one file per session.

## Links

- ML Automation Agent demo: https://smolagents-ml-automation-agent.hf.space
- ML Automation Agent CLI: https://github.com/huggingface/ml-automation-agent

## Data description

Each `*.jsonl` file contains a single ML Automation Agent session converted to a
...
ML Automation Agent applies automated best-effort scrubbing for common secret patterns
