...
class Config(BaseModel):
    ...
    session_dataset_repo: str = "smolagents/ml-automation-agent-sessions"
    ...
    personal_trace_repo_template: str = "{hf_user}/ml-automation-agent-sessions"
...
USER_CONFIG_ENV_VAR = "ML_AUTOMATION_AGENT_CLI_CONFIG"
DEFAULT_USER_CONFIG_PATH = (
    Path.home() / ".config" / "ml-automation-agent" / "cli_agent_config.json"
)
...
def _load_user_config() -> dict[str, Any]:
    ...
    raw_path = os.environ.get(USER_CONFIG_ENV_VAR) or os.environ.get("ML_INTERN_CLI_CONFIG")
    if raw_path:
    ...
    # Support legacy default path (~/.config/ml-intern/...)
    legacy_path = Path.home() / ".config" / "ml-intern" / "cli_agent_config.json"
    if DEFAULT_USER_CONFIG_PATH.exists():
        return _load_json_config(DEFAULT_USER_CONFIG_PATH)
    if legacy_path.exists():
        return _load_json_config(legacy_path)
    return {}
...
def apply_slack_user_defaults(raw_config: dict[str, Any]) -> dict[str, Any]:
    ...
    # Respect either new or legacy env var for enabling Slack defaults
    if os.environ.get("ML_AUTOMATION_AGENT_SLACK_NOTIFICATIONS") is None and os.environ.get(
        "ML_INTERN_SLACK_NOTIFICATIONS"
    ) is None:
        # neither set — default True
        pass
    else:
        # If either is explicitly set to a falsey value, disable
        if not _env_bool("ML_AUTOMATION_AGENT_SLACK_NOTIFICATIONS", _env_bool("ML_INTERN_SLACK_NOTIFICATIONS", True)):
            return raw_config
    ...
    destination_name = (
        os.environ.get("ML_AUTOMATION_AGENT_SLACK_DESTINATION") or os.environ.get("ML_INTERN_SLACK_DESTINATION") or SLACK_DEFAULT_DESTINATION
    ).strip()
    ...
    auto_events = _env_list("ML_AUTOMATION_AGENT_SLACK_AUTO_EVENTS") or _env_list("ML_INTERN_SLACK_AUTO_EVENTS")
