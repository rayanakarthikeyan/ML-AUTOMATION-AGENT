...
client end-to-end.
It is skipped unless ``ML_AUTOMATION_AGENT_LIVE_SANDBOX_TESTS=1`` and ``HF_TOKEN`` are set.
"""
...
if env_file := os.environ.get("ML_AUTOMATION_AGENT_LIVE_ENV_FILE"):
    load_dotenv(Path(env_file))


def _skip_without_live_sandbox() -> None:
    if os.environ.get("ML_AUTOMATION_AGENT_LIVE_SANDBOX_TESTS") != "1":
        pytest.skip("set ML_AUTOMATION_AGENT_LIVE_SANDBOX_TESTS=1 to create a real sandbox")
...
def test_live_sandbox_authenticated_agent_communication():
    ...
    try:
        sandbox = Sandbox.create(
            owner=owner,
            name="ml-automation-agent-live-auth",
            hardware="cpu-basic",
            private=True,
            token=token,
        ...
        write = sandbox.write("/tmp/ml_automation_agent_live_auth.txt", "alpha\nbeta\n")
        assert write.success, write.error

        exists = sandbox._call("exists", {"path": "/tmp/ml_automation_agent_live_auth.txt"})
        assert exists.success, exists.error
        assert exists.output == "true"

        read = sandbox.read("/tmp/ml_automation_agent_live_auth.txt")
        ...
        try:
            reread = reattached.read("/tmp/ml_automation_agent_live_auth.txt")
