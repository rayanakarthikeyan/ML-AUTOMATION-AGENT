...
def test_artifact_collection_title_uses_session_date_and_id():
    assert (
        ...
        == "ml-automation-agent-artifacts-2026-05-05-session-123"
    )
...
def test_artifact_collection_title_uses_short_uuid_fragment():
    ...
    assert title == "ml-automation-agent-artifacts-2026-05-05-fadcbc77"
    assert len(title) < 60
...
def test_artifact_collection_title_still_truncates_long_non_uuid_ids():
    ...
    assert title.startswith("ml-automation-agent-artifacts-2026-05-05-custom-session-id")
    assert len(title) < 60
...
def test_register_hub_artifact_creates_private_collection_and_adds_item_once(
    ...
    class FakeApi:
        ...
        def create_collection(self, **kwargs):
            ...
            return SimpleNamespace(slug="alice/ml-automation-agent-artifacts")
    ...
    assert b"ml-automation-agent" in api.uploads[0]["path_or_fileobj"]
...
def test_sitecustomize_bootstrap_is_valid_python():
    ...
    assert "ml-automation-agent-artifacts-2026-05-05-session-123" in code
...
def test_sitecustomize_bootstrap_reuses_existing_collection_slug():
    ...
    setattr(
        ...
        "alice/ml-automation-agent-artifacts-2026-05-05-session-123",
    )
    ...
    compile(code, "sitecustomize.py", "exec")
    assert (
        "collection_slug = 'alice/ml-automation-agent-artifacts-2026-05-05-session-123'" in code
    )
...
def test_sitecustomize_caches_lazy_collection_slug_across_bootstraps(
    ...
    collection_slug = "alice/ml-automation-agent-artifacts-2026-05-05-session-123"
    uploads = []
    downloads = []
    collection_creates = []
    collection_items = []
