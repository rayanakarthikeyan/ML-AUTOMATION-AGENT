...
def test_dataset_repo_card_exposes_each_upload_as_config():
    card = dataset_uploads.dataset_repo_card(
        "alice/ml-automation-agent-s1-datasets",
        [
            "README.md",
...
async def test_push_dataset_upload_creates_private_repo_and_uploads_file(monkeypatch):
    ...
    assert api.create_calls == [
        {
            "repo_id": "alice/ml-automation-agent-12345678-datasets",
            "repo_type": "dataset",
            "private": True,
            "exist_ok": True,
        }
    ...
    assert result.repo_id == "alice/ml-automation-agent-12345678-datasets"
    assert result.config_name == "upload_feedfacecafe"
    assert result.format == "csv"
    assert result.load_dataset_snippet == (
        "from datasets import load_dataset\n\n"
        'dataset = load_dataset("alice/ml-automation-agent-12345678-datasets", '
...
async def test_upload_route_appends_context_note_and_persists(monkeypatch):
    ...
    uploaded = dataset_uploads.DatasetUpload(
        session_id="s1",
        repo_id="alice/ml-automation-agent-s1-datasets",
        repo_type="dataset",
        private=True,
        upload_id="abc123",
        ...
        size_bytes=14,
        format="jsonl",
        hub_url="https://huggingface.co/datasets/alice/ml-automation-agent-s1-datasets/blob/main/uploads/abc123/rows.jsonl",
