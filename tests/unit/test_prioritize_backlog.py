...
def test_hf_discussion_event_normalization():
    ...
    discussion = SimpleNamespace(
        num=7,
        repo_id="smolagents/ml-automation-agent",
        repo_type="space",
        title="Space fails",
        status="open",
        author="alice",
    ...
    assert record["url"] == (
        "https://huggingface.co/spaces/smolagents/ml-automation-agent/discussions/7"
    )
...
def test_cli_defaults_without_live_network_or_llm():
    ...
    assert args.github_repo == "huggingface/ml-automation-agent"
    assert args.hf_space == "smolagents/ml-automation-agent"
