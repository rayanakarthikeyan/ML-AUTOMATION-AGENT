...
def test_dataset_card_readme_has_metadata_and_public_warning():
    readme = dataset_card_readme("lewtun/ml-automation-agent-sessions")

    assert readme.startswith("---\n")
    assert 'pretty_name: "ML Automation Agent Session Traces"' in readme
    assert "task_categories:\n- text-generation" in readme
    assert "- agent-traces" in readme
    assert "- coding-agent" in readme
    assert "- ml-automation-agent" in readme
    assert 'path: "sessions/**/*.jsonl"' in readme
    assert "ML Automation Agent demo: https://smolagents-ml-automation-agent.hf.space" in readme
    assert "ML Automation Agent CLI: https://github.com/huggingface/ml-automation-agent" in readme
...
def test_upload_dataset_card_only_for_claude_code_format():
    ...
    _upload_dataset_card(api, "lewtun/ml-automation-agent-sessions", "hf_token", "row")
    assert api.calls == []

    _upload_dataset_card(api, "lewtun/ml-automation-agent-sessions", "hf_token", "claude_code")
    assert len(api.calls) == 1
    assert api.calls[0]["path_in_repo"] == "README.md"
    assert api.calls[0]["repo_id"] == "lewtun/ml-automation-agent-sessions"
...
def test_update_upload_status_preserves_other_uploader_fields(tmp_path):
    ...
    _update_upload_status(
        ...
        "personal_upload_url",
        "success",
        "https://huggingface.co/datasets/user/ml-automation-agent-sessions",
    )
