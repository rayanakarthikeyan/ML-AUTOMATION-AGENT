...
def session_dataset_repo_id(hf_username: str | None, session_id: str) -> str:
    ...
    return f"{namespace}/ml-automation-agent-{safe_session_id[:8]}-datasets"
...
Private dataset files uploaded through ML Automation Agent.

Files are stored under `uploads/<upload_id>/` and are attached to the
corresponding ML Automation Agent session context by Hub reference, not by copying file
...
async def push_dataset_upload_to_hub(
    *,
    ...
    await asyncio.to_thread(
        ...
        repo_id=repo_id,
        repo_type="dataset",
        commit_message="Update ML Automation Agent dataset upload configs",
    )
