...
Environment::
    ...
    KPI_SOURCE_REPO     default smolagents/ml-automation-agent-sessions
    KPI_TARGET_REPO     default smolagents/ml-automation-agent-kpis
    ML_AUTOMATION_AGENT_KPIS_DISABLED  if truthy, the scheduler is not started
"""
...
async def _run_hour(hour_dt: datetime) -> None:
    ...
    try:
        ...
        api = HfApi()
        source = os.environ.get("KPI_SOURCE_REPO", "smolagents/ml-automation-agent-sessions")
        target = os.environ.get("KPI_TARGET_REPO", "smolagents/ml-automation-agent-kpis")
...
def start(backfill_hours: int = 6) -> None:
    ...
    global _scheduler
    if os.environ.get("ML_AUTOMATION_AGENT_KPIS_DISABLED"):
        logger.info("kpis_scheduler: disabled via ML_AUTOMATION_AGENT_KPIS_DISABLED")
        return
